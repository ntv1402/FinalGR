...
namespace HeneGames.DialogueSystem
{
    public class DialogueManager : MonoBehaviour
    {
        ...
        private void PlaySound(string _audioClip)
        {
            ...
            //Play sentence sound
            AudioManager.Instance.PlayAmbient(_audioClip, 0);
        }
        ...
        public void NoMoreTalking()
        {
            Collider2D collider = GetComponent<Collider2D>();
            collider.enabled = false;
        }

        public void BidenBlast()
        {
            GameObject player = GameObject.FindGameObjectWithTag("Player");
            if (player != null) 
            { 
                player.GetComponent<PlayerHealthManager>().TakeDamage(9999);
            }
        }
    }
